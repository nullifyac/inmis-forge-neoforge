...
public class BackpackInfo {
     ...
     * @param numberOfRows number of horizontal slot groups
     */
    public static BackpackInfo of(String name, int rowWidth, int numberOfRows, boolean isFireImmune, Holder<SoundEvent> openSound) {
        SoundEvent soundEvent = openSound.value();
        String soundId = BuiltInRegistries.SOUND_EVENT.getKey(soundEvent) != null
                ? BuiltInRegistries.SOUND_EVENT.getKey(soundEvent).toString()
                : "minecraft:item.armor.equip_leather";
        return new BackpackInfo(name, rowWidth, numberOfRows, isFireImmune, soundId);
    }

    public static BackpackInfo of(String name, int rowWidth, int numberOfRows, boolean isFireImmune, Holder<SoundEvent> openSound, boolean dyeable) {
        SoundEvent soundEvent = openSound.value();
        String soundId = BuiltInRegistries.SOUND_EVENT.getKey(soundEvent) != null
                ? BuiltInRegistries.SOUND_EVENT.getKey(soundEvent).toString()
                : "minecraft:item.armor.equip_leather";
