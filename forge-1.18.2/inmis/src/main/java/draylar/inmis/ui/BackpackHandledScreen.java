...
public class BackpackHandledScreen extends AbstractContainerScreen<BackpackScreenHandler> {
    ...
    private static final Component UPGRADES_LABEL = new TranslatableComponent("inmis.gui.backpack_upgrades");
    private static final Component UPGRADES_TOOLTIP = new TranslatableComponent("inmis.gui.backpack_upgrades.tooltip");
    private static final Component FILTERS_LABEL = new TranslatableComponent("inmis.gui.filters");
    private static final Component SET_FILTERS_LABEL = new TranslatableComponent("inmis.gui.set_filters");
    ...
    private List<FormattedCharSequence> queuedTooltip;
    ...
    protected void renderBg(PoseStack poseStack, float delta, int mouseX, int mouseY) {
        GraphicsAdapter graphics = new GraphicsAdapter(poseStack);
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float delta) {
        ...
        if (queuedTooltip != null) {
            this.renderTooltip(poseStack, queuedTooltip, mouseX, mouseY);
        } else {
    ...
    private void renderAugmentSettings(GraphicsAdapter graphics, SettingsLayout layout, int mouseX, int mouseY,
        ...
        Component settingsTitle = new TranslatableComponent("inmis.gui.upgrade_settings", selectedAugment.label());
    ...
    private void renderFunnellingSettings(GraphicsAdapter graphics, SettingsLayout layout, int startY, int mouseX, int mouseY,
        ...
        Component modeValue = new TranslatableComponent("augment.backpacked.funnelling.mode." + settings.mode().getSerializedName());
        Component label = new TranslatableComponent("augment.backpacked.funnelling.mode", modeValue);
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, startY, this.font.width(label), 10)) {
            Component tooltip = new TranslatableComponent("augment.backpacked.funnelling.mode." + settings.mode().getSerializedName() + ".tooltip");
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, buttonY, this.font.width(SET_FILTERS_LABEL), 10)) {
            queueTooltip(new TranslatableComponent("inmis.gui.set_filters.tooltip"));
        }
    ...
    private void renderQuiverlinkSettings(GraphicsAdapter graphics, SettingsLayout layout, int startY, int mouseX, int mouseY,
        ...
        Component label = new TranslatableComponent("augment.backpacked.quiverlink.priority");
        Component value = new TranslatableComponent("augment.backpacked.quiverlink.priority." + settings.priority().getSerializedName());
        Component text = new TextComponent(label.getString() + ": ").append(value);
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, startY, this.font.width(text), 10)) {
            queueTooltip(new TranslatableComponent("augment.backpacked.quiverlink.priority.tooltip"));
        }
    }
    ...
    private void renderLootboundSettings(GraphicsAdapter graphics, SettingsLayout layout, int startY, int mouseX, int mouseY,
        ...
        int y = startY;
        Component blocks = new TranslatableComponent("augment.backpacked.lootbound.blocks");
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, y, this.font.width(blocks), 10)) {
            queueTooltip(new TranslatableComponent("augment.backpacked.lootbound.blocks.tooltip"));
        }
        y += 12;
        Component mobs = new TranslatableComponent("augment.backpacked.lootbound.mobs");
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, y, this.font.width(mobs), 10)) {
            queueTooltip(new TranslatableComponent("augment.backpacked.lootbound.mobs.tooltip"));
        }
    }
    ...
    private void renderLightweaverSettings(GraphicsAdapter graphics, SettingsLayout layout, int startY, int mouseX, int mouseY,
        ...
        Component label = new TranslatableComponent("augment.backpacked.lightweaver.light_level");
        Component text = new TextComponent(label.getString() + ": " + settings.minimumLight());
        graphics.drawString(this.font, text, layout.contentX, startY, 0xFFD8C6B2, false);
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, startY, this.font.width(text), 10)) {
            queueTooltip(new TranslatableComponent("augment.backpacked.lightweaver.light_level.tooltip"));
        }
        int y = startY + 12;
        Component sound = new TranslatableComponent("augment.backpacked.lightweaver.place_sound");
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, y, this.font.width(sound), 10)) {
            queueTooltip(new TranslatableComponent("augment.backpacked.lightweaver.place_sound.tooltip"));
        }
    }
    ...
    private void renderSeedflowSettings(GraphicsAdapter graphics, SettingsLayout layout, int startY, int mouseX, int mouseY,
        ...
        int y = startY;
        Component randomize = new TranslatableComponent("augment.backpacked.seedflow.randomize_seeds");
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, y, this.font.width(randomize), 10)) {
            queueTooltip(new TranslatableComponent("augment.backpacked.seedflow.randomize_seeds.tooltip"));
        }
        y += 12;
        Component useFilters = new TranslatableComponent("augment.backpacked.seedflow.use_filters");
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, y, this.font.width(useFilters), 10)) {
            queueTooltip(new TranslatableComponent("augment.backpacked.seedflow.use_filters.tooltip"));
        }
        y += 12;
        graphics.drawString(this.font, FILTERS_LABEL, layout.contentX, y, 0xFFD8C6B2, false);
    ...
    private void renderHopperBridgeSettings(GraphicsAdapter graphics, SettingsLayout layout, int startY, int mouseX, int mouseY,
        ...
        int y = startY;
        Component insert = new TranslatableComponent("augment.backpacked.hopper_bridge.insert");
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, y, this.font.width(insert), 10)) {
            queueTooltip(new TranslatableComponent("augment.backpacked.hopper_bridge.insert.tooltip"));
        }
        y += 12;
        Component extract = new TranslatableComponent("augment.backpacked.hopper_bridge.extract");
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, y, this.font.width(extract), 10)) {
            queueTooltip(new TranslatableComponent("augment.backpacked.hopper_bridge.extract.tooltip"));
        }
        y += 12;
        Component modeLabel = new TranslatableComponent("augment.backpacked.hopper_bridge.filter_mode");
        Component modeValue = new TranslatableComponent("augment.backpacked.hopper_bridge.filter_mode." + settings.filterMode().getSerializedName());
        Component text = new TextComponent(modeLabel.getString() + ": ").append(modeValue);
        ...
        if (isWithin(mouseX, mouseY, layout.contentX, y, this.font.width(text), 10)) {
            queueTooltip(new TranslatableComponent("augment.backpacked.hopper_bridge.filter_mode.tooltip"));
        }
        y += 12;
        graphics.drawString(this.font, FILTERS_LABEL, layout.contentX, y, 0xFFD8C6B2, false);
        renderFilterGrid(graphics, layout, y + 10, settings.filters(), mouseX, mouseY);
    ...
    private boolean handleAugmentSettingsClick(SettingsLayout layout, double mouseX, double mouseY, int button) {
        ...
        switch (selectedAugment) {
            case FUNNELLING -> {
                Component modeLabel = new TranslatableComponent("augment.backpacked.funnelling.mode",
                        new TranslatableComponent("augment.backpacked.funnelling.mode." + augments.funnelling().mode().getSerializedName()));
            ...
            case QUIVERLINK -> {
                Component label = new TranslatableComponent("augment.backpacked.quiverlink.priority");
                Component value = new TranslatableComponent("augment.backpacked.quiverlink.priority." + augments.quiverlink().priority().getSerializedName());
                Component text = new TextComponent(label.getString() + ": ").append(value);
                if (isWithin(mouseX, mouseY, layout.contentX, y, this.font.width(text), 10)) {
            ...
            case HOPPER_BRIDGE -> {
                ...
                y += 12;
                Component modeLabel = new TranslatableComponent("augment.backpacked.hopper_bridge.filter_mode");
                Component modeValue = new TranslatableComponent("augment.backpacked.hopper_bridge.filter_mode." + augments.hopperBridge().filterMode().getSerializedName());
                Component text = new TextComponent(modeLabel.getString() + ": ").append(modeValue);
                if (isWithin(mouseX, mouseY, layout.contentX, y, this.font.width(text), 10)) {
    ...
    private void queueTooltip(Component tooltip) {
        queuedTooltip = this.font.split(tooltip, 200);
    }

    private void queueTooltip(List<FormattedCharSequence> tooltip) {
    ...
    private void applyScissor(int x1, int y1, int x2, int y2) {
        ...
        int scissorY = (int) Math.floor(this.minecraft.getWindow().getHeight() - y2 * scale);
        int scissorWidth = (int) Math.ceil((x2 - x1) * scale);
        int scissorHeight = (int) Math.ceil((y2 - y1) * scale);
    ...
    private final class GraphicsAdapter {
        ...
        private GraphicsAdapter(PoseStack poseStack) {
        ...
        private void blit(ResourceLocation texture, int x, int y, int u, int v, int width, int height, int texWidth, int texHeight) {
            RenderSystem.setShaderTexture(0, texture);
            BackpackHandledScreen.this.blit(poseStack, x, y, u, v, width, height, texWidth, texHeight);
        }

        private void fill(int x1, int y1, int x2, int y2, int color) {
            BackpackHandledScreen.this.fill(poseStack, x1, y1, x2, y2, color);
        }

        private void drawString(Font font, Component text, int x, int y, int color, boolean shadow) {
            if (shadow) {
        ...
        private void drawString(Font font, String text, int x, int y, int color, boolean shadow) {
            if (shadow) {
                font.drawShadow(poseStack, text, x, y, color);
            } else {
                font.draw(poseStack, text, x, y, color);
